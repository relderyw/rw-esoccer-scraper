...
def map_league_name(name: str) -> str:
    # Sua função original de mapeamento de ligas (cole aqui toda a sua lógica)
    # Exemplo simples:
...
async def fetch_event_details(event_id: str) -> Dict:
    ...
    async with httpx.AsyncClient(timeout=10) as session:
        ...
        # HT geralmente não vem separado, mas se tiver
        ht_home = ht_away = 0  # fallback, ajustável
...
async def fetch_event_tracker_info(event_id: str) -> Dict | None:
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as session:
            ...
            r.raise_for_status()
            data = r.json()
            # Ajuste conforme o JSON real (você pode printar para ver a estrutura)
            score = data.get("score", [0, 0])
        ...
        print(f"[WARN] Tracker falhou para {event_id}: {e}")
        return None
        ...
        try:
            async with httpx.AsyncClient(timeout=15) as session:
                ...
                for event_id in finished_ids:
                    ...
                    placar_final = {
                        "ft_home": last_score["home"],
                        "ft_away": last_score["away"],
                        "ht_home": last_score["ht_home"],
                        "ht_away": last_score["ht_away"]
                    }
                    ...
                    try:
                        ...
                        print(f"[DEBUG] GetEventDetails falhou, tentando tracker")
                    ...
                    home_nick = extract_pure_nick_canonical(details.get("home_raw", "") if details else "")
                    away_nick = extract_pure_nick_canonical(details.get("away_raw", "") if details else "")
                    
                    doc = {
                        "event_id": event_id,
                        "league_mapped": map_league_name(details.get("league", "") if details else ""),
                        "home_nick": home_nick or "UNKNOWN",
                        "away_nick": away_nick or "UNKNOWN",
